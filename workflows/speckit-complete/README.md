...
| テスト | 入力 | 期待動作 | 結果 | 判定 |
|-------|------|----------|------|------|
| 番号のみ | `1` | constitution処理 | エラー発生 | ❌ |
| 番号+内容 | `2 ユーザー認証機能の要件を整理` | specify処理 | 詳細な要件整理を出力 | ✅ |
| 自然言語 | `ECサイトのタスクを分解したい` | tasks処理 | タスク分解を出力 | ✅ |
| 無効番号 | `8` | エラーメッセージ | Opening Statement再表示 | ⚠️ |
| 意味不明 | `あいうえお` | エラーメッセージ | 不適切な応答 | ❌ |

### 課題

1. **番号のみの入力が処理できない** - 「1」だけ入力するとエラーになる
2. **無効入力のエラーハンドリング不足** - 意味不明な入力に対して適切なエラーメッセージを返さない

### 修正が必要な箇所

- Command Parser: 番号のみの入力を処理できるようにする
- Question Classifier: 無効な入力を検出してError Handlerにルーティング
